...
import {title, dateToMinutes} from "es6!common";
...
var PAGE_SIZE = config("playlistPageSize"),
    ...
    isPageLoading = true,
    mBus,
    onPlaylistEndReached, onPlaylistTopReached;
...
function activateCurrent($this) {
    ...
    var scrollTop = $("#jukebox .playlistContainer").scrollTop(),
        top = _.reduce($this.prevAll("div"), function (total, el) {
...
function _displayTracks(pageSize, pagedRes, append, back) {
    ...
    $.each(tracks, function (i, track) {
        var method = back ? "appendTo" : "prependTo",
            $track = $("#trackTemplate")
                .clone()
                .prop("id", "")
                .attr("data-id", track.id);

        $track[method]("#jukebox .playlist");

        $(".num", $track).html(currentPage + (i + direction) * direction + ".");
        $(".name", $track).html(title(track));
        $(".duration", $track).html(dateToMinutes(track.duration));
...
function displayTracks(pagedRes, back) {
    try {
...
onPlaylistEndReached = function onPlaylistEndReached(e, el) {
...
onPlaylistTopReached = function onPlaylistTopReached(e, el) {
